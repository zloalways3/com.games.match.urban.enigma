...
public class AnimationBehaviour_UrbanEnigma : MonoBehaviour
{

	//[FormerlySerializedAs("gameObjectToMove")]
	public GameObject gameObjectToMove_UrbanEnigma;
	//[FormerlySerializedAs("targetPosition")]
	public Transform targetPosition_UrbanEnigma;
	//[FormerlySerializedAs("moveSpeed")]
	public float moveSpeed_UrbanEnigma = 1f;
	//[FormerlySerializedAs("easeType")]
	public Ease easeType_UrbanEnigma = Ease.Linear;
	//[FormerlySerializedAs("moveOnXAxis")]
	public bool moveOnXAxis_UrbanEnigma = true;
	//[FormerlySerializedAs("moveOnYAxis")]
	public bool moveOnYAxis_UrbanEnigma = true;
	//[FormerlySerializedAs("gameObjectToMove2")]
	public GameObject gameObjectToMove2_UrbanEnigma;
	//[FormerlySerializedAs("targetPosition2")]
	public Transform targetPosition2_UrbanEnigma;
	//[FormerlySerializedAs("on_first_object_start")]
	public UnityEvent on_first_object_start_UrbanEnigma;
	//[FormerlySerializedAs("on_first_object_finish")]
	public UnityEvent on_first_object_finish_UrbanEnigma;
	//[FormerlySerializedAs("on_second_object_start")]
	public UnityEvent on_second_object_start_UrbanEnigma;
	//[FormerlySerializedAs("on_second_object_finish")]
	public UnityEvent on_second_object_finish_UrbanEnigma;
	...
	{
		if (gameObjectToMove_UrbanEnigma != null && targetPosition_UrbanEnigma != null)
		{
			on_first_object_start_UrbanEnigma?.Invoke();

			Vector3 targetPos = new Vector3(moveOnXAxis_UrbanEnigma ? targetPosition_UrbanEnigma.position.x : gameObjectToMove_UrbanEnigma.transform.position.x,
				moveOnYAxis_UrbanEnigma ? targetPosition_UrbanEnigma.position.y : gameObjectToMove_UrbanEnigma.transform.position.y,
				gameObjectToMove_UrbanEnigma.transform.position.z);

			gameObjectToMove_UrbanEnigma.transform.DOMove(targetPos, moveSpeed_UrbanEnigma).SetEase(easeType_UrbanEnigma).OnComplete(() =>
			{
				on_first_object_finish_UrbanEnigma?.Invoke();
			});
		}
	...
	{
		if (gameObjectToMove2_UrbanEnigma != null && targetPosition2_UrbanEnigma != null)
		{
			on_second_object_start_UrbanEnigma?.Invoke();

			Vector3 targetPos2 = new Vector3(moveOnXAxis_UrbanEnigma ? targetPosition2_UrbanEnigma.position.x : gameObjectToMove2_UrbanEnigma.transform.position.x,
				moveOnYAxis_UrbanEnigma ? targetPosition2_UrbanEnigma.position.y : gameObjectToMove2_UrbanEnigma.transform.position.y,
				gameObjectToMove2_UrbanEnigma.transform.position.z);

			gameObjectToMove2_UrbanEnigma.transform.DOMove(targetPos2, moveSpeed_UrbanEnigma).SetEase(easeType_UrbanEnigma).OnComplete(() =>
			{
				on_second_object_finish_UrbanEnigma?.Invoke();
			});
		}
